...
class TfCfCustomerPdc(models.TransientModel):
    _name = 'tf.cf.customer.pdc'
    _description = "Generate Customer PDC Cash Flow"
    ...
    def action_generate(self):
        ...
        if not return_id:
            return {
                'name': "Customer PDCs: %s - %s" % (self.date_from, self.date_to),
                'type': 'ir.actions.act_window',
                'view_mode': 'pivot',
                'domain': [('id', 'in', line_ids)],
                'res_model': 'tf.cf.projection.line',
                'view_id': self.env.ref('tf_ph_cf_treasury.tf_cf_projection_line_pivot').id
            }
        else:
            return line_ids
