...
var StatementModel = BasicModel.extend({
    avoidCreate: false,
    quickCreateFields: ['account_id', 'amount', 'analytic_account_id', 'label', 'tax_ids', 'force_tax_included', 'analytic_tag_ids', 'withholding_tag_ids', 'to_check'],
    ...
    _loadTaxes: function(){
        ...
        return this._rpc({
                model: 'account.tax',
                method: 'search_read',
                fields: ['price_include', 'name', 'account_id'],
            }).then(function (taxes) {
    ...
    updateProposition: function (handle, values) {
        ...
        if ('withholding_tax_id' in values) {
            prop.label = 'Withholding Tax - ' + prop.withholding_tax_id.display_name;
	    prop.account_id = values.account_id = this._formatNameGet(this.taxes[values.withholding_tax_id.id].account_id);
        }
    ...
    _formatQuickCreate: function (line, values) {
        ...
        var prop = {
            ...
            'withholding_tax_id': this._formatNameGet(values.withholding_tax_id),
    ...
    _formatToProcessReconciliation: function (line, prop) {
        ...
        var result = {
            ...
            withholding_tax_id: prop.withholding_tax_id
        };
        if (!isNaN(prop.id)) {
