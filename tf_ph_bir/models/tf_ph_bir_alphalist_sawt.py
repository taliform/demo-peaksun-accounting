...
class AccountAlphalistSawt(models.Model):
    _name = 'account.alphalist.sawt'
    _description = 'Summary Alphalist of Withholding Taxes'
    ...
    @api.model
    def get_title(self):
        return _('Summary Alphalist of Withholding Taxes')
    ...
    def get_report_type(self):
        return self.env.ref('tf_ph_bir.account_report_type_alphalist_sawt')

    def _get_templates(self):
        templates = super(AccountAlphalistSawt, self)._get_templates()
        templates['line_template'] = 'tf_ph_bir.line_template_alphalist_sawt_report'
        return templates
    ...
    def get_csv(self, options):
        ...
        for detail in company_data:
            ...
            if detail['vat']:
                ...
                detail_branch = str(detail['vat']).replace('-', '')[9:13]
    ...
    def get_report_filename(self, options):
        ...
        if 'export_csv' in options and options['export_csv']:
            ...
            return f"{company_data['company_tin']}{company_data['date']}1702Q.dat"
    ...
    def _get_lines(self, options, line_id=None):
        ...
        domain = [('date', '<=',date_to),
                  ('date', '>=',date_from),
                  ('tax_line_id', '!=', False),
                  ('move_id.type','not in', ('in_invoice', 'in_refund', 'in_receipt')),
                  ('move_id.state', '!=', 'draft')]
        ...
        if not line_id or partner_ids2:
            total_line = {
                'id': 'overall_sawt_partners_total',
                'name': _('Total'),
