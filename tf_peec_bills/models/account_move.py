...
class AccountMove(models.Model):
    ...
    withholding_tax_ids = fields.Many2many(
        'account.tax', 'move_wht_tax_rel', 'move_id', 'tax_id',
        string='Withholding Tax', domain="[('type_tax_use','=','purchase'),('for_withholding','=',True)]")

    @api.onchange('partner_id', 'company_id')
    def _onchange_partner_id(self):
        res = super(AccountMove, self)._onchange_partner_id()
        for rec in self.filtered_domain([
            ('type', 'in', ['in_invoice', 'in_refund', 'in_receipt']),
        ]):
            if rec.partner_id:
                rec.withholding_tax_ids = self.partner_id.withholding_tax_ids.ids or False
            else:
...
class AccountMoveLine(models.Model):
    ...
    def _get_computed_taxes(self):
        ...
        res = res | self.move_id.withholding_tax_ids
        return res
